<template>
  <div class="tuijian" v-if="hotBlogData.length > 0">
    ...
    <ul class="tjpic" v-if="hotBlogData[0]">
      <i><img alt="" style="cursor:pointer" v-if="hotBlogData[0].photoList" :src="hotBlogData[0].photoList[0]"
              @click="goToInfo(hotBlogData[0])"></i>
    ...
    <ul class="sidenews">
      <li v-for="(item, index) in sideNews" v-if="index != 0" :key="item.uid">
        <i><img alt="" style="cursor:pointer" v-if="item.photoList" :src="item.photoList[0]"
                @click="goToInfo(item)"></i>
...
export default {
  ...
  computed: {
    ...
    sideNews() {
      // return this.hotBlogData.filter(blog => this.hotBlogData.indexOf(blog) !== -1);
      return this.hotBlogData;
    }
  },
  methods: {
    //跳转到文章详情【或推广链接】
